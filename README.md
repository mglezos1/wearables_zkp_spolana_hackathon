# 🧬 VITRAM: Zero-Knowledge Proof Smart Health Monitoring System

VITRAM is a privacy-first web3 biometric wellness platform built on Solana. It allows individuals to securely verify their health and anxiety metrics to authorized verifiers or health insurance providers **without ever revealing their raw medical data**.
...
When you access the VITRAM application, the flow is separated into two simple perspectives: the **Data Generator** and the **Data Verifier**.

### 1. Generating Your Health Proof
1. **Connect Your Wallet:** Click the "Connect Phantom" button in the top right of the Smart Health Monitoring System dashboard.
...
1. **Access the Portal:** Navigate to the **🏢 View Wellness Portal** via the navigation menu.
2. **Input Signature:** Paste the Solana Transaction Signature produced directly into the secure box.
3. **Automated Verification:** Click "Verify". The portal will query the blockchain, instantly decouple the mathematical ZK-proof, and automatically issue an "ALL CLEAR" or "SEEK SUPPORT" status based entirely on cryptographic math—with zero exposure to the actual underlying heart rate data!
